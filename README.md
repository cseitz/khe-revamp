...
![I've seen enough. I'm satisfied](https://i.kym-cdn.com/entries/icons/facebook/000/033/069/jojo.jpg "hi")

# Installation
Be sure NodeJS is installed. *Built using Node v13.12.0 and NPM 6.14.4*
...
When developing the frontend, you can live-test your changes by using `npm run serve`. To build the project for production (when you are finished making changes), you can run `npm run build`. This compiles the Vue project and places it inside of a folder named `dist` in the root of the project. This directory can be served to a desired port via the included `server.js`, however it is recommended to only do this for archiving purposes. Instead, the backend is capable of serving the site and it also provides performance logging and statistics viewable on the staff site. The `server.js` inside of frontend is only capable of serving the site (such as previous year's KHE sites) and lacks all unnecessary features.

All of the above also applies to the staff site which is located inside backend. By default, the backend will serve the staff site that is stored inside of `backend/staff/dist`.
